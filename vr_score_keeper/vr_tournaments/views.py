...
def index(request):
    ...
    if latest_tournament:
        ...
        for player in players:
            score = player.score_set.filter(tournament=latest_tournament).aggregate(total_score=Sum('score'))['total_score']
            player_scores.append((player, score))
    else:
        player_scores = []
    return render(request, 'index.html', {'tournaments': tournaments, 'latest_tournament': latest_tournament, 'player_scores': player_scores})
def players(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('players.html', {'players': players, 'form': form})
        else:
...
def tournament_list(request):
    tournaments = Tournament.objects.all()
    form = TournamentForm()
    return render(request, 'tournament_list.html', {'tournaments': tournaments, 'form': form})
